...
# Generated by running ./bundle_tarball.sh %%{version}
Source0:        v%{version}-bundled.tar.xz
Source5:        immich-cli.tar.xz

# Service files
Source1:        sysusers
Source2:        systemd.service
Source3:        systemd-ml.service
Source4:        environment
...
sed -i -e 's|^lib_path=.*|lib_path=%{_libdir}/libmimalloc.so.2|' \
  server/start.sh machine-learning/start.sh
...
cp -pr immich-cli/lib/node_modules/@immich %{buildroot}%{nodejs_sitelib}
...
%post
%systemd_post %{name}.service %{name}-ml.service

%preun
%systemd_preun %{name}.service %{name}-ml.service

%postun
%systemd_postun_with_restart %{name}.service %{name}-ml.service
